...
class iCore50(iData):
    ...
    def download_data(self):
        ...
        print("hey") # 여기까지 오래 걸림
        # train_x = np.concatenate(imgs)
        train_x = np.vstack(imgs)
        # 이것도 꽤 걸리는데 앞에꺼보다는 덜 걸림
        print("hi")
        # train_y = np.concatenate(labels)
        train_y = np.hstack(labels)
        self.train_data = train_x
        self.train_targets = train_y

        test_x, test_y = datagen.get_test_set() # 오래 걸림
