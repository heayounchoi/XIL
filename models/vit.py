...
from timm.models._builder import build_model_with_cfg, resolve_pretrained_cfg, adapt_input_conv
...
class Block(nn.Module):
    ...
    def __init__(
            self, dim, num_heads, current_block, last_block, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, total_sessions=6):
        super().__init__()
        ...
        # dsa_hidden_dim = int(dim/mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.current_block = current_block
        # self.last_block = last_block
        # if self.current_block == self.last_block-1:
            # self.dsa_pool = nn.ModuleList([
                # Mlp(in_features=dim, hidden_features=dsa_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, 
                # dsa_idx=None, 
                # task_idx=None,
                # extract_vector=False
                ):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        # if not dsa_idx and not extract_vector:
            # if self.current_block == self.last_block-1:
                # x = x + self.drop_path2(self.ls2(self.dsa_pool[dsa_idx](self.norm2(x))))
        return x
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, current_block= i, last_block=depth)
