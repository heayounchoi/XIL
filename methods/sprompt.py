...
class SPrompts(BaseLearner):
    ...
    def train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            logging.info(info)
            print('Task {}, Epoch {}/{} => Loss {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}'.format(
                self._cur_task, epoch + 1, self.run_epoch, losses / len(train_loader), train_acc, test_acc))
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                for id, task_centers in enumerate(self.all_keys):
                    X.append(task_centers.detach().cpu().numpy())
                    Y.append(np.array([id]*len(task_centers)))

                X = np.concatenate(X,0)
                Y = np.concatenate(Y,0)
                neigh = KNeighborsClassifier(n_neighbors=5, metric='l1')
                neigh.fit(X, Y)
                ...
                selection = torch.tensor(selection).to(self._device)*0
